...
const MessagesView: React.FC<MessagesViewProps> = ({ initialMessages, onClose }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-white rounded-2xl overflow-hidden relative">
      {/* Header for Messages View */}
      <div className="p-4 sm:p-6 border-b border-gray-100 flex items-center bg-white shrink-0 sticky top-0 z-20">
        <button onClick={onClose} className="p-2 rounded-full hover:bg-gray-100 text-gray-500 transition-colors" aria-label="Back to main view">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
        </button>
        <h2 className="text-2xl font-bold text-gray-800 ml-3">Messages</h2>
        <p className="text-sm text-gray-500 ml-3 hidden sm:block">Communicate with drivers and system directly.</p>
      </div>

      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar List */}
        <div className="w-full sm:w-1/3 border-r border-gray-100 flex flex-col bg-gray-50/50">
          <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-white">
            <h3 className="font-bold text-lg text-gray-800">Conversations</h3>
            <button className="text-emerald-600 hover:bg-emerald-50 p-2 rounded-lg" aria-label="Compose new message">
              ...
              <>
                  <div className="p-4 border-b border-gray-100 flex justify-between items-center shadow-sm z-10">
                      ...
                      <div className="flex gap-2 text-gray-400">
                          <button className="p-2 hover:bg-gray-100 rounded-lg" aria-label="Call sender"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg></button>
                          <button className="p-2 hover:bg-gray-100 rounded-lg" aria-label="Video call sender"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg></button>
                      </div>
                  </div>

                  <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50/30 custom-scrollbar">
                      <div className="flex justify-start">
                          <div className="bg-gray-100 rounded-2xl rounded-tl-sm p-4 max-w-md text-sm text-gray-700" aria-label={`Message from ${selectedMessage.sender}`}>
                              {selectedMessage.content}
                              <div className="text-[10px] text-gray-400 mt-1 text-right">{selectedMessage.time}</div>
                          </div>
                      </div>
                      <div className="flex justify-end">
                          <div className="bg-emerald-600 rounded-2xl rounded-tr-sm p-4 max-w-md text-sm text-white shadow-md shadow-emerald-200" aria-label="Reply to sender">
                  ...
                  <form onSubmit={handleSend} className="p-4 border-t border-gray-100 bg-white">
                      <div className="flex gap-2">
                          ...
                          />
                          <button type="submit" className="p-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 shadow-md transition-colors" aria-label="Send message">
